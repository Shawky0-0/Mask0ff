...
For race, TOCTOU, idempotency, double-spend, partial-construction, or concurrent-state work, run the dedicated race planner first. Require an invariant and reset, two clean sequential baselines, negotiated-protocol evidence, protocol-correct bounded delivery, asynchronous settlement, authoritative final state, and retry/session/jitter/eventual-consistency controls. Never label a thread barrier as single-packet or last-byte synchronization, and never replace synchronization with production request volume.
